...
@if($permit->module_setting_group_create)
<a href="{{url('/groups/create')}}" class="btn btn-primary">Create</a>
@endif
...
<table class="table table-striped table-bordered table-hover" id="example-2">
     ...
     @foreach ($groups as $group)
         <tr>
             ...
             <td>@if($group->group_status == 1) 
                     Active 
                 @else 
                     In-Active 
                 @endif
             </td>
             <td><a href="{{url('groups',$group->group_id)}}" class="btn btn-primary btn-sm btn-icon icon-left">Read</a></td>
             <td>@if($permit->module_setting_group_update)
		    <a href="{{route('groups.edit',$group->group_id)}}" class="btn btn-secondary btn-sm btn-icon icon-left">Update</a>
		 @endif
	     </td>
             <td>
		@if($permit->module_setting_group_delete)	
             {!! Form::open(['method' => 'DELETE', 'route'=>['groups.destroy', $group->group_id]]) !!}
