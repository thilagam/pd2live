...
@if($permit->module_product_upload_delivery)
	<div class="col-sm-12">
		...
		<div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title">{{ $dictionary['upload_delivery_file'] }}</h3>
			...
			<div class="panel-body">
				...
				{!! Form::open(['url' => '/product/deliveryupload/'.$prodConfigs['delivery']['pconf_product_id'],'method'=>'post','id'=>'productref','class'=>'form-horizontal','enctype'=>'multipart/form-data']) !!}	
			
				{!! Form::hidden('pconf_item_id', $prodConfigs['delivery']['pconf_item_id']) !!}
						    ...
						    <div class="input-group">
				                <span class="input-group-btn">
				                    <span class="btn btn-primary btn-file">
				                      {{ $dictionary['fprwd_upload'] }}<input type="file" name="file_delivery_1" multiple>
				                    </span>
				                </span>
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							...
							{!! Form::select('pdn_ref',
					        			    $alphaRange,
	        							   	$prodConfigs['delivery']['pconf_reference_id'],
	        							    [	
	        								'class'=>'form-control',
	        								'id'=>'pdn_ref',
...
@if($permit->module_product_upload_validate_delivery)
	<div class="col-sm-12">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title">{{ $dictionary['validate_delivery_file'] }}</h3>
			...
			jQuery(document).ready(function($)
			{
				...
				<table class="table table-bordered table-striped" id="example-2">
					...
					@foreach($prodConfigs['delivery_uploads_verify'] as $ruv)
					...
					<tbody class="middle-align">
						  <tr>
						  	...
						  	<td><a href="{{ url('/download/'.Crypt::encrypt($ruv->upload_url).'/s') }}" class="btn btn-info btn-sm btn-icon icon-left">
								...
								<a href="{{ url('/product/approveupload/'.$ruv->upload_id) }}" class="btn btn-success btn-sm btn-icon icon-left">
...
@endif
	...
	<div class="col-sm-12">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title">{{ $dictionary['old_delivery_file'] }}</h3>
			</div>
			<div class="panel-body">
			<script type="text/javascript">
				jQuery(document).ready(function($)
				...
				<table class="table table-bordered table-striped" id="example-3">
					...
					<tbody class="middle-align">
						@foreach($prodConfigs['delivery_uploads_verifed'] as $ruv)
						  <tr>
							<td>{{ $ruv->dt }}</td>
						  	<td>{{ $ruv->up_first_name }} {{ $ruv->up_last_name }} - [ {{ $ruv->upload_by }} ]</td>
						  	<td>{{ $ruv->upload_original_name }}</td>
